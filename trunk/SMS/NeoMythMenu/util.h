...
#define puts_shared(STR,X,Y,ATTR) pfn_puts_asm(STR,ATTR,(0x3000 + (X << 1) + (Y << 6)))

extern void putsn_asm(const char *str,BYTE attributes,BYTE max_chars,WORD vram_addr);
#define putsn(STR,X,Y,ATTR,MAX_CHRS) putsn_asm(STR,ATTR,MAX_CHRS,(0x3000 + (X << 1) + (Y << 6)))
...
extern BYTE memcmp_asm(const BYTE* a,const BYTE* b,WORD len);
#endif
