...
#define MAX_SETTINGS_FILE_SIZE 768
...
{
	char *buf = (char*)&gbaCardAlphabeticalIdx[0];
	WORD bytesRead;
	int pos, i;

	if (pf_open("/MENU/SNES/SETTINGS.TXT") == FR_OK)
	{
		if (pf_read(buf, MAX_SETTINGS_FILE_SIZE, &bytesRead) == FR_OK)
		{
			buf[MAX_SETTINGS_FILE_SIZE] = 0;
			...
			pos = strstri("FAST_CORE_ACCESS=", buf);
			if (pos >= 0)
			{
				if (buf[pos+17] == '1') sdLoaderMemSel = 1;
			}
			pos = strstri("SAVE_LAST_GAME_INFO=", buf);
			if (pos >= 0)
			{
				if (buf[pos+20] == '0') allowLastGameSave = 0;
			}
			pos = strstri("USE_SD_AUTO_BUFFER=", buf);
			if (pos >= 0)
			{
				if (buf[pos+19] == '0') useSdAutoBuffer = 0;
			}
